...
class CROMAEncoderWrapper(nn.Module):

    """
    A wrapper for CROMA models from torchgeo to return only the forward pass of the encoder 
    Attributes:
        croma_model (CROMA): The instantiated croma model
    Methods:
        forward(x: List[torch.Tensor], wavelengths: list[float]) -> torch.Tensor:
            Forward pass for embeddings with specified indices.
    """

    def __init__(self, croma_model, wavelengths, weights=None, out_indices=None) -> None:
        """
        Args:
    ...
    def forward(self, x: List[torch.Tensor], **kwargs) -> torch.Tensor:
        ...
        x, _ = self.croma_model.patch_embed(x, wavelist)
        x = x + self.croma_model.pos_embed[:, 1:, :]
        # append cls token
